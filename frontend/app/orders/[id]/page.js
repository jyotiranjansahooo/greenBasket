...
export default function OrderDetailsPage({ params }) {
  ...
  const currentStep = order ? orderSteps.indexOf(order.status) : -1;
  ...
  if (isPending) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#F7FAF5]">
        <h1 className="text-3xl font-bold text-[#346739]">Loading order...</h1>
      </main>
    );
  }
  ...
  if (error || !order) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#F7FAF5]">
        <h1 className="text-3xl text-red-500">Order not found</h1>
      </main>
    );
  }
          ...
          <div className="rounded-3xl bg-white p-8 shadow-lg">
            ...
            <div className="mt-10 grid gap-6 md:grid-cols-2">
              <div className="rounded-2xl bg-gray-50 p-5">
                <h3 className="mb-3 text-xl font-bold">📍 Delivery Address</h3>

                <div className="space-y-2 text-sm text-gray-700">
                  <p>
                    <span className="font-semibold">House / Flat:</span>{" "}
                    {order.deliveryAddress?.houseNumber || "N/A"}
                  </p>

                  <p>
                    <span className="font-semibold">Street / Area:</span>{" "}
                    {order.deliveryAddress?.street || "N/A"}
                  </p>

                  <p>
                    <span className="font-semibold">Landmark:</span>{" "}
                    {order.deliveryAddress?.landmark || "N/A"}
                  </p>
                ...
                <p className="mt-3 text-gray-500">Slot: {order.deliverySlot}</p>
              </div>
              ...
              <div className="rounded-2xl bg-gray-50 p-5">
                ...
                <p className="mt-2">Status: {order.paymentStatus}</p>
              </div>
            </div>
            ...
            <div className="mt-10">
              <h3 className="mb-5 text-2xl font-bold">Products</h3>
